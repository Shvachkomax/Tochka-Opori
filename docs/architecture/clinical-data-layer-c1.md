...
Observation database identity is `module + owner_type + owner_id + source_type + source_id + concept + source_event_key`, with `NULLS NOT DISTINCT` preserved for nullable `source_id`. Supersession lineage is the same identity without `source_event_key`; `source_event_key` identifies the revision within that lineage. The writer scopes both predecessor lookup and duplicate recovery by the complete subject and source lineage. A logical source hash is useful for deterministic grouping but is not an ownership boundary, so source-id collisions across owners or modules cannot create a supersession relationship.
