-- Phase 11C.1 stabilization: include owner-identity assignments in accept scope.
...
BEGIN
  ...
  IF FOUND THEN
    IF v_existing.primary_expert_id != v_expert_id THEN
      RETURN jsonb_build_object('ok', false, 'error', 'У вас уже есть активный специалист в этой организации. Обратитесь к администратору для перевода.', 'code', 'ASSIGNMENT_CONFLICT');
    END IF;

    UPDATE public.patient_specialist_invitations
    SET status = 'accepted',
        accepted_at = v_now,
        target_owner_id = CASE WHEN direction = 'specialist_to_patient' THEN p_owner_id ELSE target_owner_id END,
        target_owner_type = CASE WHEN direction = 'specialist_to_patient' THEN 'anonymous_case' ELSE target_owner_type END,
        updated_at = v_now
  ...
  INSERT INTO public.patient_assignments (
    public_code, organization_id, primary_expert_id, assigned_by_expert_id,
    assigned_by_expert_name, module, status, source
  ) VALUES (
    p_public_code, v_invitation.organization_id, v_expert_id, v_expert_id,
    'invitation', v_invitation.module, 'active', 'patient_invitation_accept'
  );
