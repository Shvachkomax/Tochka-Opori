...
${doctor.replace(/===DOCTOR_REPORT===/g, "").trim().split("\n").map(l => `<p>${l}</p>`).join("\n")}
  ...
  // Validate council invite token on mount
  React.useEffect(() => {
    if (councilInviteToken) {
      setCouncilInvitePageLoading(true);
      fetch("/api/council", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "validateInviteToken", token: councilInviteToken }),
      })
        .then(r => r.json())
        .then(data => {
          if (data.ok && data.invitation) {
            setCouncilInvitePageData(data.invitation);
          } else {
            setCouncilInvitePageError(data.error || "Приглашение недействительно");
          }
        })
        .catch(() => setCouncilInvitePageError("Ошибка проверки приглашения"))
        .finally(() => setCouncilInvitePageLoading(false));
    }
  }, [councilInviteToken]);
  ...
  // Clinical Council admin state
  const [councilAdminTab, setCouncilAdminTab] = useState("invitations"); // invitations | candidates | experts
  const [councilInvitations, setCouncilInvitations] = useState([]);
  const [councilInvitationsCount, setCouncilInvitationsCount] = useState(0);
  const [councilInvitationsLoading, setCouncilInvitationsLoading] = useState(false);
  const [councilInvitationDetail, setCouncilInvitationDetail] = useState(null);
  const [councilExperts, setCouncilExperts] = useState([]);
  const [councilExpertsCount, setCouncilExpertsCount] = useState(0);
  const [councilExpertsLoading, setCouncilExpertsLoading] = useState(false);
  const [councilExpertDetail, setCouncilExpertDetail] = useState(null);
  const [councilExpertDetailOpen, setCouncilExpertDetailOpen] = useState(false);
  const [councilInviteForm, setCouncilInviteForm] = useState({ first_name: "", last_name: "", email: "", specialty: "", organization: "", notes: "", expires_days: 30 });
  const [councilInviteSaving, setCouncilInviteSaving] = useState(false);
  const [councilInviteResult, setCouncilInviteResult] = useState(null);
  const [councilActionLoading, setCouncilActionLoading] = useState(null);
  const [councilTokenCopied, setCouncilTokenCopied] = useState(false);

  // Expert invite page state (for /expert-invite/<token>)
  const [councilInviteToken, setCouncilInviteToken] = useState(() => {
    try {
      const m = window.location.pathname.match(/^\/expert-invite\/([a-zA-Z0-9]+)/);
      return m ? m[1] : null;
    } catch { return null; }
  });
  const [councilInvitePageData, setCouncilInvitePageData] = useState(null);
  const [councilInvitePageLoading, setCouncilInvitePageLoading] = useState(false);
  const [councilInvitePageError, setCouncilInvitePageError] = useState(null);
  const [councilInviteFormFields, setCouncilInviteFormFields] = useState({
    first_name: "", last_name: "", email: "", phone: "", specialty: "", position: "", organization: "", professional_note: "", public_name_consent: false,
  });
  const [councilInviteFormSaving, setCouncilInviteFormSaving] = useState(false);
  const [councilInviteFormSuccess, setCouncilInviteFormSuccess] = useState(false);

  // Expert cabinet state (for /expert)
  const [councilExpertToken, setCouncilExpertToken] = useState(() => {
    try {
      const m = window.location.pathname.match(/^\/expert\/?$/);
      return m ? (localStorage.getItem("tochka_council_expert_token") || null) : null;
    } catch { return null; }
  });
  const [councilExpertData, setCouncilExpertData] = useState(null);
  const [councilExpertLoginInput, setCouncilExpertLoginInput] = useState("");
  const [councilExpertLoggingIn, setCouncilExpertLoggingIn] = useState(false);
  ...
  // --- Clinical Council admin functions ---

  async function adminLoadCouncilInvitations() {
    setCouncilInvitationsLoading(true);
    try {
      const res = await fetch("/api/admin", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      const data = await res.json();
      if (data.ok) {
        setCouncilInvitations(data.records || []);
        setCouncilInvitationsCount(data.count || 0);
      }
    } catch {} finally {
      setCouncilInvitationsLoading(false);
    }
  }

  async function adminLoadCouncilExperts() {
    setCouncilExpertsLoading(true);
    try {
      const res = await fetch("/api/admin", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      const data = await res.json();
      if (data.ok) {
        setCouncilExperts(data.records || []);
        setCouncilExpertsCount(data.count || 0);
      }
    } catch {} finally {
      setCouncilExpertsLoading(false);
    }
  }

  async function adminCouncilAction(action, id, extra = {}) {
    setCouncilActionLoading(action + id);
    try {
      const res = await fetch("/api/admin", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (data.ok) {
        if (action === "createCouncilInvitation") {
          setCouncilInviteResult(data.invitation);
          setCouncilInviteForm({ first_name: "", last_name: "", email: "", specialty: "", organization: "", notes: "", expires_days: 30 });
          adminLoadCouncilInvitations();
          return data.invitation;
        }
        if (action === "approveCouncilExpert") {
          showToast("Эксперт утверждён. Токен доступа: " + data.access_token, "success", 15000);
          ...
          showToast("Эксперт приостановлен", "success");
        } else if (action === "restoreCouncilExpert") {
          showToast("Эксперт восстановлен", "success");
        }
        adminLoadCouncilInvitations();
        adminLoadCouncilExperts();
      } else {
        showToast(data.error || "Ошибка", "error");
      }
    } catch {
...
`}</style>
          ...
          {["intake", "diary", "trash", "reviews"].map(tab => (
            ...
            <>
              ...
              <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
                {["invitations", "candidates", "experts"].map(tab => (
                  <button
                    key={tab}
                    style={{
                      border: 0, borderRadius: 14, padding: "8px 16px", fontWeight: 700, fontSize: 13, cursor: "pointer",
                      background: councilAdminTab === tab ? t.tabActive : t.tabBg,
                      color: councilAdminTab === tab ? t.tabActiveText : t.text,
                    }}
                    onClick={() => {
                      setCouncilAdminTab(tab);
                      setCouncilExpertDetailOpen(false);
                      if (tab === "invitations") adminLoadCouncilInvitations();
                      else if (tab === "candidates") adminLoadCouncilExperts();
                      else if (tab === "experts") adminLoadCouncilExperts();
                    }}
                  >
                    {tab === "invitations" ? "Приглашения" : tab === "candidates" ? "Кандидаты" : "Эксперты"}
                  </button>
                ))}
              </div>

              {/* Invitations tab */}
              {councilAdminTab === "invitations" && (
                <div>
                  {/* Create invitation form */}
                  <div style={{ background: t.cardBg, border: `1px solid ${t.cardBorder}`, borderRadius: 20, padding: 24, marginBottom: 20 }}>
                    <h3 style={{ fontSize: 16, fontWeight: 700, marginBottom: 16 }}>Новое приглашение</h3>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 12 }}>
                      <input placeholder="Имя *" value={councilInviteForm.first_name} onChange={e => setCouncilInviteForm({ ...councilInviteForm, first_name: e.target.value })}
                        style={{ border: `1px solid ${t.inputBorder}`, borderRadius: 12, background: t.inputBg, color: t.inputText, padding: "10px 14px", fontSize: 14, outline: "none" }} />
                      <input placeholder="Фамилия *" value={councilInviteForm.last_name} onChange={e => setCouncilInviteForm({ ...councilInviteForm, last_name: e.target.value })}
                        style={{ border: `1px solid ${t.inputBorder}`, borderRadius: 12, background: t.inputBg, color: t.inputText, padding: "10px 14px", fontSize: 14, outline: "none" }} />
                      <input placeholder="Email" value={councilInviteForm.email} onChange={e => setCouncilInviteForm({ ...councilInviteForm, email: e.target.value })}
                        style={{ border: `1px solid ${t.inputBorder}`, borderRadius: 12, background: t.inputBg, color: t.inputText, padding: "10px 14px", fontSize: 14, outline: "none" }} />
                      <input placeholder="Специальность" value={councilInviteForm.specialty} onChange={e => setCouncilInviteForm({ ...councilInviteForm, specialty: e.target.value })}
                        style={{ border: `1px solid ${t.inputBorder}`, borderRadius: 12, background: t.inputBg, color: t.inputText, padding: "10px 14px", fontSize: 14, outline: "none" }} />
                      <input placeholder="Организация" value={councilInviteForm.organization} onChange={e => setCouncilInviteForm({ ...councilInviteForm, organization: e.target.value })}
                        style={{ border: `1px solid ${t.inputBorder}`, borderRadius: 12, background: t.inputBg, color: t.inputText, padding: "10px 14px", fontSize: 14, outline: "none" }} />
                      <input placeholder="Срок (дней)" type="number" min="1" value={councilInviteForm.expires_days} onChange={e => setCouncilInviteForm({ ...councilInviteForm, expires_days: parseInt(e.target.value) || 30 })}
                        style={{ border: `1px solid ${t.inputBorder}`, borderRadius: 12, background: t.inputBg, color: t.inputText, padding: "10px 14px", fontSize: 14, outline: "none" }} />
                    </div>
                    <textarea placeholder="Заметка (необязательно)" value={councilInviteForm.notes} onChange={e => setCouncilInviteForm({ ...councilInviteForm, notes: e.target.value })}
                      style={{ width: "100%", border: `1px solid ${t.inputBorder}`, borderRadius: 12, background: t.inputBg, color: t.inputText, padding: "10px 14px", fontSize: 14, outline: "none", resize: "vertical", minHeight: 60, marginBottom: 12 }} />
                    <button
                      disabled={councilInviteSaving || !councilInviteForm.first_name || !councilInviteForm.last_name}
                      style={{
                        border: 0, borderRadius: 14, padding: "12px 24px", fontWeight: 700, fontSize: 14, cursor: councilInviteSaving ? "wait" : "pointer",
                        background: councilInviteSaving || !councilInviteForm.first_name || !councilInviteForm.last_name ? t.tabBg : t.accent,
                        color: councilInviteSaving || !councilInviteForm.first_name || !councilInviteForm.last_name ? t.muted : "#fff",
                      }}
                      onClick={async () => {
                        setCouncilInviteSaving(true);
                        await adminCouncilAction("createCouncilInvitation", null, { ...councilInviteForm });
                        setCouncilInviteSaving(false);
                      }}
                    >
                      {councilInviteSaving ? "Создание..." : "Создать приглашение"}
                    </button>
                  </div>
                  ...
                  {councilInviteResult && (
                    <div style={{ background: "#E2EBE4", border: "1px solid rgba(125,154,137,.3)", borderRadius: 16, padding: 20, marginBottom: 20 }}>
                      ...
                      <div style={{ fontSize: 13, color: "#5F7D6C", marginBottom: 4 }}>Код: <strong>{councilInviteResult.code}</strong></div>
                      <div style={{ fontSize: 12, color: "#5F7D6C", marginBottom: 10, wordBreak: "break-all" }}>
                        Ссылка: {window.location.origin}/expert-invite/{councilInviteResult.token}
                      </div>
                      <div style={{ display: "flex", gap: 8 }}>
                        <button
                          style={{ border: 0, borderRadius: 10, padding: "6px 14px", fontWeight: 700, fontSize: 12, cursor: "pointer", background: "#5F7D6C", color: "#fff" }}
                          onClick={() => {
                            navigator.clipboard.writeText(window.location.origin + "/expert-invite/" + councilInviteResult.token);
                            setCouncilTokenCopied(true);
                            setTimeout(() => setCouncilTokenCopied(false), 2000);
                          }}
                        >
                          {councilTokenCopied ? "✓ Скопировано" : "Копировать ссылку"}
                        </button>
                        <button
                          ...
                          onClick={() => setCouncilInviteResult(null)}
                        >
                          Закрыть
                        </button>
                      </div>
                  ...
                  {/* Invitations list */}
                  <div style={{ background: t.cardBg, border: `1px solid ${t.cardBorder}`, borderRadius: 20, padding: 24 }}>
                    <h3 style={{ fontSize: 16, fontWeight: 700, marginBottom: 12 }}>Приглашения ({councilInvitationsCount})</h3>
                    {councilInvitationsLoading ? (
                      <div style={{ color: t.muted, fontSize: 14, padding: 20 }}>Загрузка...</div>
                    ) : councilInvitations.length === 0 ? (
                      <div style={{ color: t.muted, fontSize: 14, padding: 20 }}>Нет приглашений</div>
                    ) : (
                      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                        {councilInvitations.map(inv => (
                          <div key={inv.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 16px", borderRadius: 14, background: t.cardBg, border: `1px solid ${t.cardBorder}` }}>
                            <div>
                              <span style={{ fontWeight: 700, fontSize: 14 }}>{inv.invited_first_name} {inv.invited_last_name}</span>
                              <span style={{ marginLeft: 10, fontSize: 12, color: t.muted }}>{inv.invite_code}</span>
                              {inv.invited_email && <span style={{ marginLeft: 10, fontSize: 12, color: t.muted }}>· {inv.invited_email}</span>}
                              <div style={{ fontSize: 12, color: t.muted, marginTop: 2 }}>
                                Статус: <span style={{ fontWeight: 600, color: inv.status === "accepted" ? "#5F7D6C" : inv.status === "revoked" ? "#ef4444" : inv.status === "expired" ? "#eab308" : t.text }}>{inv.status}</span>
                            ...
                            <div style={{ display: "flex", gap: 6 }}>
                              {inv.status !== "revoked" && inv.status !== "accepted" && (
                                <button
                                  disabled={councilActionLoading === "revokeCouncilInvitation" + inv.id}
                                  ...
                                  onClick={() => adminCouncilAction("revokeCouncilInvitation", inv.id)}
                                >
                                  Отозвать
                                </button>
                              )}
              ...
              {/* Candidates tab (pending_review experts) */}
              {councilAdminTab === "candidates" && (
                <div>
                  <div style={{ background: t.cardBg, border: `1px solid ${t.cardBorder}`, borderRadius: 20, padding: 24 }}>
                    <h3 style={{ fontSize: 16, fontWeight: 700, marginBottom: 12 }}>Кандидаты на рассмотрении ({councilExperts.filter(e => e.status === "pending_review").length})</h3>
                    {councilExpertsLoading ? (
                      <div style={{ color: t.muted, fontSize: 14, padding: 20 }}>Загрузка...</div>
                    ) : councilExperts.filter(e => e.status === "pending_review").length === 0 ? (
                      <div style={{ color: t.muted, fontSize: 14, padding: 20 }}>Нет кандидатов</div>
                    ) : (
                      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                        {councilExperts.filter(e => e.status === "pending_review").map(exp => (
                          <div key={exp.id} style={{ padding: "12px 16px", borderRadius: 14, background: t.cardBg, border: `1px solid ${t.cardBorder}` }}>
                            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
                              <div>
                                <span style={{ fontWeight: 700, fontSize: 14 }}>{exp.first_name} {exp.last_name}</span>
                                {exp.specialty && <span style={{ marginLeft: 10, fontSize: 12, color: t.muted }}>· {exp.specialty}</span>}
                                {exp.organization && <span style={{ marginLeft: 10, fontSize: 12, color: t.muted }}>· {exp.organization}</span>}
                              </div>
                              <div style={{ display: "flex", gap: 6 }}>
                                <button
                                  disabled={councilActionLoading === "approveCouncilExpert" + exp.id}
                                  style={{ border: 0, borderRadius: 10, padding: "6px 12px", fontWeight: 700, fontSize: 12, cursor: "pointer", background: "#5F7D6C", color: "#fff" }}
                                  onClick={() => adminCouncilAction("approveCouncilExpert", exp.id)}
                                >
                                  {councilActionLoading === "approveCouncilExpert" + exp.id ? "..." : "Утвердить"}
                                </button>
                                <button
                                  disabled={councilActionLoading === "rejectCouncilExpert" + exp.id}
                                  style={{ border: 0, borderRadius: 10, padding: "6px 12px", fontWeight: 700, fontSize: 12, cursor: "pointer", background: "transparent", color: "#ef4444", border: "1px solid #ef4444" }}
                                  onClick={() => adminCouncilAction("rejectCouncilExpert", exp.id)}
                                >
                                  {councilActionLoading === "rejectCouncilExpert" + exp.id ? "..." : "Отклонить"}
                                </button>
                              </div>
                            </div>
                            {exp.email && <div style={{ fontSize: 12, color: t.muted }}>Email: {exp.email}</div>}
                            {exp.professional_note && <div style={{ fontSize: 12, color: t.muted, marginTop: 4 }}>{exp.professional_note}</div>}
                          </div>
                        ))}
                      </div>
                    )}
              ...
              {/* Experts tab (active/paused) */}
              {councilAdminTab === "experts" && (
                <div>
                  <div style={{ display: "flex", gap: 8, marginBottom: 12, justifyContent: "flex-end" }}>
                    <button
                      disabled={councilExpertsLoading}
                      style={{ border: 0, borderRadius: 10, padding: "6px 14px", fontWeight: 700, fontSize: 12, cursor: "pointer", background: t.tabBg, color: t.text }}
                      onClick={() => adminCouncilAction("exportCouncilExperts", null)}
                    >
                      Выгрузить JSONL
                    </button>
                  </div>
                  <div style={{ background: t.cardBg, border: `1px solid ${t.cardBorder}`, borderRadius: 20, padding: 24 }}>
                    <h3 style={{ fontSize: 16, fontWeight: 700, marginBottom: 12 }}>Эксперты ({councilExperts.filter(e => e.status === "active" || e.status === "paused").length})</h3>
                    {councilExpertsLoading ? (
                      <div style={{ color: t.muted, fontSize: 14, padding: 20 }}>Загрузка...</div>
                    ) : councilExperts.filter(e => e.status === "active" || e.status === "paused").length === 0 ? (
                      <div style={{ color: t.muted, fontSize: 14, padding: 20 }}>Нет экспертов</div>
                    ) : (
                      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                        {councilExperts.filter(e => e.status === "active" || e.status === "paused").map(exp => (
                          <div key={exp.id} style={{ padding: "12px 16px", borderRadius: 14, background: t.cardBg, border: `1px solid ${t.cardBorder}` }}>
                            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 4 }}>
                              ...
                              <div style={{ display: "flex", gap: 6 }}>
                                {exp.status === "active" && (
                                  <button
                                    disabled={councilActionLoading === "pauseCouncilExpert" + exp.id}
                                    ...
                                    onClick={() => adminCouncilAction("pauseCouncilExpert", exp.id)}
                                  >
                                    {councilActionLoading === "pauseCouncilExpert" + exp.id ? "..." : "Приостановить"}
                                  </button>
                                )}
                                {exp.status === "paused" && (
                                  <button
                                    disabled={councilActionLoading === "restoreCouncilExpert" + exp.id}
                                    style={{ border: 0, borderRadius: 10, padding: "6px 12px", fontWeight: 700, fontSize: 12, cursor: "pointer", background: "#5F7D6C", color: "#fff" }}
                                    onClick={() => adminCouncilAction("restoreCouncilExpert", exp.id)}
                                  >
                                    {councilActionLoading === "restoreCouncilExpert" + exp.id ? "..." : "Восстановить"}
                                  </button>
                                )}
                              </div>
                            </div>
                            {exp.email && <div style={{ fontSize: 12, color: t.muted }}>Email: {exp.email}</div>}
                            {exp.approved_at && <div style={{ fontSize: 12, color: t.muted }}>Утверждён: {new Date(exp.approved_at).toLocaleDateString("ru-RU")}</div>}
                          </div>
                        ))}
                      </div>
                    )}
    ...
    return (
      <div style={{ minHeight: "100vh", background: "#F6F0E7", color: "#2E2A25", fontFamily: "Inter, system-ui, sans-serif", padding: 32 }}>
        <div style={{ maxWidth: 600, margin: "0 auto" }}>
          ...
          {councilInvitePageData && !councilInviteFormSuccess && (
            <div style={{ background: "#fff", border: "1px solid rgba(46,42,37,.1)", borderRadius: 20, padding: 28 }}>
              <div style={{ fontSize: 14, color: "#7A7268", marginBottom: 16 }}>
                {councilInvitePageData.first_name && <span>Приглашаем: <strong>{councilInvitePageData.first_name} {councilInvitePageData.last_name}</strong></span>}
                {councilInvitePageData.specialty && <span style={{ marginLeft: 8 }}>· {councilInvitePageData.specialty}</span>}
              </div>
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                <div style={{ display: "flex", gap: 12 }}>
                  <input placeholder="Имя *" value={councilInviteFormFields.first_name} onChange={e => setCouncilInviteFormFields({ ...councilInviteFormFields, first_name: e.target.value })}
                  ...
                  <input placeholder="Фамилия *" value={councilInviteFormFields.last_name} onChange={e => setCouncilInviteFormFields({ ...councilInviteFormFields, last_name: e.target.value })}
                    style={{ flex: 1, border: "1px solid rgba(46,42,37,.15)", borderRadius: 12, background: "#fff", color: "#2E2A25", padding: "12px 14px", fontSize: 14, outline: "none" }} />
                </div>
                <input placeholder="Email *" type="email" value={councilInviteFormFields.email} onChange={e => setCouncilInviteFormFields({ ...councilInviteFormFields, email: e.target.value })}
                ...
                <input placeholder="Телефон (необязательно)" value={councilInviteFormFields.phone} onChange={e => setCouncilInviteFormFields({ ...councilInviteFormFields, phone: e.target.value })}
                  style={{ border: "1px solid rgba(46,42,37,.15)", borderRadius: 12, background: "#fff", color: "#2E2A25", padding: "12px 14px", fontSize: 14, outline: "none" }} />
                <input placeholder="Специальность" value={councilInviteFormFields.specialty} onChange={e => setCouncilInviteFormFields({ ...councilInviteFormFields, specialty: e.target.value })}
                  style={{ border: "1px solid rgba(46,42,37,.15)", borderRadius: 12, background: "#fff", color: "#2E2A25", padding: "12px 14px", fontSize: 14, outline: "none" }} />
                <input placeholder="Должность" value={councilInviteFormFields.position} onChange={e => setCouncilInviteFormFields({ ...councilInviteFormFields, position: e.target.value })}
                  style={{ border: "1px solid rgba(46,42,37,.15)", borderRadius: 12, background: "#fff", color: "#2E2A25", padding: "12px 14px", fontSize: 14, outline: "none" }} />
                <input placeholder="Организация" value={councilInviteFormFields.organization} onChange={e => setCouncilInviteFormFields({ ...councilInviteFormFields, organization: e.target.value })}
                  style={{ border: "1px solid rgba(46,42,37,.15)", borderRadius: 12, background: "#fff", color: "#2E2A25", padding: "12px 14px", fontSize: 14, outline: "none" }} />
                <textarea placeholder="Профессиональная заметка (необязательно)" value={councilInviteFormFields.professional_note} onChange={e => setCouncilInviteFormFields({ ...councilInviteFormFields, professional_note: e.target.value })}
                ...
                <label style={{ display: "flex", alignItems: "flex-start", gap: 10, fontSize: 13, color: "#7A7268", cursor: "pointer" }}>
                  <input type="checkbox" checked={councilInviteFormFields.public_name_consent} onChange={e => setCouncilInviteFormFields({ ...councilInviteFormFields, public_name_consent: e.target.checked })}
                    style={{ marginTop: 2, width: 18, height: 18 }} />
                  <span>Разрешаю упоминание моего имени в списке экспертов (необязательно)</span>
                </label>
                <button
                  disabled={councilInviteFormSaving || !councilInviteFormFields.first_name || !councilInviteFormFields.last_name || !councilInviteFormFields.email}
                  style={{
                    border: 0, borderRadius: 14, padding: "14px 24px", fontWeight: 800, fontSize: 15, cursor: councilInviteFormSaving ? "wait" : "pointer",
                    background: councilInviteFormSaving || !councilInviteFormFields.first_name || !councilInviteFormFields.last_name || !councilInviteFormFields.email ? "#ccc" : "#B85C4A",
                    color: "#fff", marginTop: 8,
                  }}
                  onClick={async () => {
                    setCouncilInviteFormSaving(true);
                    try {
                      const res = await fetch("/api/council", {
                        method: "POST",
                        headers: { "Content-Type": "application/json" },
                        body: JSON.stringify({ action: "acceptInvite", token: councilInviteToken, ...councilInviteFormFields }),
                      });
                      const data = await res.json();
                      if (data.ok) {
                        setCouncilInviteFormSuccess(true);
                      } else {
                        setCouncilInvitePageError(data.error || "Ошибка отправки");
                      }
                    } catch {
                      setCouncilInvitePageError("Ошибка сети");
                    } finally {
                      setCouncilInviteFormSaving(false);
                    }
                  }}
                >
                  {councilInviteFormSaving ? "Отправка..." : "Отправить заявку"}
                </button>
              </div>
            </div>
          )}
  ...
  if (window.location.pathname.match(/^\/expert\/?$/)) {
    return (
      <div style={{ minHeight: "100vh", background: "#F6F0E7", color: "#2E2A25", fontFamily: "Inter, system-ui, sans-serif", padding: 32 }}>
        <div style={{ maxWidth: 800, margin: "0 auto" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 32 }}>
            ...
            {councilExpertData && (
              <button
                onClick={() => { setCouncilExpertData(null); setCouncilExpertToken(null); localStorage.removeItem("tochka_council_expert_token"); }}
          ...
          {!councilExpertData ? (
            <div style={{ background: "#fff", border: "1px solid rgba(46,42,37,.1)", borderRadius: 20, padding: 28, maxWidth: 400 }}>
              ...
              <input
                ...
                value={councilExpertLoginInput}
                onChange={e => setCouncilExpertLoginInput(e.target.value)}
                onKeyDown={e => e.key === "Enter" && (async () => {
                  setCouncilExpertLoggingIn(true);
                  try {
                    const res = await fetch("/api/council", {
                      method: "POST",
                      headers: { "Content-Type": "application/json" },
                      body: JSON.stringify({ action: "validateExpertToken", token: councilExpertLoginInput }),
                    });
                    const data = await res.json();
                    if (data.ok && data.expert) {
                      setCouncilExpertData(data.expert);
                      setCouncilExpertToken(councilExpertLoginInput);
                      localStorage.setItem("tochka_council_expert_token", councilExpertLoginInput);
                    } else {
                      showToast(data.error || "Неверный токен", "error");
                    }
                  } catch { showToast("Ошибка сети", "error"); }
                  finally { setCouncilExpertLoggingIn(false); }
                })()}
              ...
              />
              <button
                disabled={councilExpertLoggingIn || !councilExpertLoginInput}
                style={{
                  width: "100%", border: 0, borderRadius: 14, padding: "14px", fontWeight: 800, fontSize: 15, cursor: "pointer",
                  background: councilExpertLoggingIn || !councilExpertLoginInput ? "#ccc" : "#B85C4A", color: "#fff",
                }}
                onClick={async () => {
                  setCouncilExpertLoggingIn(true);
                  try {
              ...
              >
                {councilExpertLoggingIn ? "Вход..." : "Войти"}
              </button>
            </div>
          ) : (
            <div>
              ...
              <div style={{ background: "#fff", border: "1px solid rgba(46,42,37,.1)", borderRadius: 20, padding: 28, marginBottom: 20 }}>
                <h2 style={{ fontSize: 18, fontWeight: 700, marginBottom: 4 }}>{councilExpertData.first_name} {councilExpertData.last_name}</h2>
                <div style={{ fontSize: 13, color: "#7A7268", marginBottom: 12 }}>
                  {councilExpertData.specialty && <span>{councilExpertData.specialty}</span>}
                  {councilExpertData.organization && <span> · {councilExpertData.organization}</span>}
                </div>
                <div style={{ display: "flex", gap: 12, fontSize: 13, color: "#7A7268" }}>
                  <span>Статус: <strong style={{ color: councilExpertData.status === "active" ? "#5F7D6C" : "#eab308" }}>{councilExpertData.status}</strong></span>
                  {councilExpertData.approved_at && <span>Утверждён: {new Date(councilExpertData.approved_at).toLocaleDateString("ru-RU")}</span>}
                </div>
              </div>
