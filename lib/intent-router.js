 ...
 * @param {object} context - Dynamic context (wallet_balance, practice_count, etc.)
 * @returns {object|null} - Intent result with answer, intent_type, cta, etc.
 */
export function detectIntent(text, config, context) {
  ...
  // Priority 3: Service FAQ intents (product/billing questions)
  if (config.faqIntents) {
    for (const intent of config.faqIntents) {
      for (const pat of intent.patterns) {
        if (pat.test(lower)) {
          // Dynamic answer: replace {{balance}} with actual balance if available
          let answer = intent.answer;
          if (context && context.wallet_balance != null) {
            answer = answer.replace(/\{\{balance\}\}/g, context.wallet_balance.toLocaleString("ru-RU"));
          }
          if (context && context.practice_count != null) {
            answer = answer.replace(/\{\{practice_count\}\}/g, context.practice_count);
          }
          return {
            answer,
            safety_note: null,
  ...
  // Priority 4: Human handoff (shared across modules)
