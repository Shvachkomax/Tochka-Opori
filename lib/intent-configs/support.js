...
// Service FAQ — deterministic answers to product/billing questions
// {{balance}} and {{practice_count}} are replaced dynamically with actual values
const SUPPORT_FAQ_INTENTS = [
  {
    topic: "credits_usage",
    patterns: [/за что.*кредит/i, /что.*списывает/i, /что(?!\s*бесплат).*платн/i, /за что.*деньги/i, /кредит.*списыв/i],
    answer: "Кредиты списываются только за обращения к AI — подробный разбор и сообщения в «Поговорим?». Просмотр кабинета, истории, практик, отметки состояния и связь со специалистом бесплатны.",
  },
  {
  ...
  },
  {
    ...
    answer: "Бесплатно: просмотр кабинета, история разговоров, отчёты, отметки состояния, «Мои практики», связь со специалистом, вход по коду продолжения и «Поговорим?».",
  },
  {
  ...
  {
    topic: "credits_test_mode",
    patterns: [/тестов.*режим/i, /тестов.*баланс/i, /деньги.*списыв/i, /реальн.*деньг/i, /настоящ.*деньг/i],
    answer: "Сервис работает в тестовом режиме. Кредиты — это внутренний счёт для учёта использования AI-функций. Реальные деньги не списываются.",
  },
  {
    ...
    answer: "Код продолжения позволяет открыть ваш кабинет на другом устройстве. Найдите его в разделе «Доступ» внизу кабинета. Если код утрачен — можно создать новый там же.",
  },
  {
    topic: "quick_chat",
    patterns: [/что.*поговорим/i, /поговорим.*что/i, /быстр.*чат/i, /коротк.*вопрос/i, /чем.*отличает/i],
    answer: "«Поговорим?» — это быстрый чат для коротких вопросов и свободного общения. Он не требует отдельного разбора и подходит для повседневных вопросов. Подробный разговор — это структурированная сессия с вопросами и отчётом.",
  },
  {
    topic: "full_conversation",
    patterns: [/подробн.*разговор/i, /полноценн.*сессия/i, /отличие.*поговорим/i, /когда.*подробн/i],
    answer: "Подробный разговор — это структурированная сессия с вопросами AI и итоговым отчётом. Она подходит для глубокого разбора состояния. «Поговорим?» — для быстрых вопросов.",
  },
  {
    topic: "practices",
    patterns: [/что.*практик/i, /зачем.*практик/i, /откуда.*практик/i, /упражнен.*откуда/i],
    answer: "Практики — это конкретные упражнения и техники, которые AI предлагает во время разговоров. Они сохраняются в кабинете, и вы можете отметить, помогли ли они.",
  },
  {
    topic: "privacy",
  ...
  },
  {
    topic: "profile_name",
    patterns: [/как.*имя/i, /измен.*имя/i, /псевдоним/i, /обращен/i, /как.*зовут/i],
    answer: "Имя можно изменить в разделе «Профиль» внизу кабинета. Оно используется только для обращения и не является персональными данными.",
  },
  {
    topic: "specialist",
    patterns: [/как.*специалист/i, /специалист.*кто/i, /врач.*кто/i, /зачем.*специалист/i],
    answer: "Специалист — это психолог или психотерапевт, который может ответить на ваш запрос. Связаться с ним можно через раздел «Специалист» в кабинете.",
  },
];
...
export const supportConfig = {
  ...
  faqIntents: SUPPORT_FAQ_INTENTS,
