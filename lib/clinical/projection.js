...
export function validateClinicalEventInput(input = {}) {
  ...
  if (!eventType || !sourceType || !sourceEventKey) return "missing_event_identity";
...
export async function recordClinicalObservation(input = {}) {
  try {
    ...
    const { data, error } = await supabase
      ...
      .select("id")
      .maybeSingle();
    if (error) logProjectionFailure({ ...input, eventType: "observation", operation: "record_observation", errorCode: error.code || "insert_failed" });
    return data?.id || null;
  } catch (error) {
...
export function validateClinicalObservationInput(input = {}) {
  const sharedError = validateClinicalEventInput({
    ...
    sourceEventKey: input.sourceEventKey || input.sourceId || "manual",
    provenance: input.provenance,
  ...
  if (Number([input.valueNumeric, input.valueText, input.valueBoolean].filter(value => value !== null && value !== undefined).length) !== 1) return "invalid_observation_value";
