...
function applyObservationLineageScope(query, input) {
  ...
  scopedQuery = input.sourceId === null || input.sourceId === undefined
    ? scopedQuery.is("source_id", null)
    : scopedQuery.eq("source_id", input.sourceId);
...
export async function recordClinicalObservation(input = {}, { supabase: injectedSupabase } = {}) {
  try {
    ...
    const supabase = injectedSupabase || getSupabase();
    let previousQuery = applyObservationLineageScope(
      supabase.from("clinical_observations").select("id"),
      input,
    )
    ...
    if (!error) return data?.id || null;
    if (error.code === "23505") {
      const lookup = applyObservationLineageScope(
        supabase.from("clinical_observations").select("id"),
        input,
