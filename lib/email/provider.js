  ...
  resend: {
    async sendEmail({ to, toName, subject, bodyText, from, replyTo }) {
      ...
      try {
        ...
        if (replyTo || process.env.COUNCIL_EMAIL_REPLY_TO) {
          payload.reply_to = replyTo || process.env.COUNCIL_EMAIL_REPLY_TO;
        }

        console.log("[email:resend] Calling Resend API, to:", to);
...
export async function sendEmail({ to, toName, subject, bodyText }) {
  ...
  console.log("[email] Called sendEmail, provider keys:", Object.keys(PROVIDERS).join(", "));
  console.log("[email] from present:", Boolean(from));
  const result = await provider.sendEmail({ to, toName, subject, bodyText, from, replyTo });
  console.log("[email] sendEmail result success:", result.success, "messageId type:", typeof result.messageId);
  return result;
}
